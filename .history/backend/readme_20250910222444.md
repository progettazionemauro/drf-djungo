...
----------
0bis) Flusso GitHub → Hetzner → Locale

Il progetto Djungō vive in tre ambienti distinti:

GitHub (master): repository centrale, sorgente di verità. Tutto il codice e le Actions partono da qui.
...
Uno dei punti critici è che le variabili cambiano da ambiente ad ambiente.
Per questo si adottano file separati:

.env.dev → locale (DEBUG attivo, DB SQLite o Postgres locale).

GitHub Secrets → Action (DATABASE_URL, SECRET_KEY, ecc.).

.env → sul server Hetzner (produzione, DEBUG=False, DB cloud).

🔑 Regola d’oro: mai committare .env.
