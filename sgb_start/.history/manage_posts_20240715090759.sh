...
add_post() {
  ...
  POST_DATE=${POST_DATE:-$(date +"%Y-%m-%dT%H:%M:%S%z" | sed 's/\(..\)$/:\1/')}
  ...
  POST_TAGS=${POST_TAGS:-"default,tag"}
  POST_CATEGORIES=${POST_CATEGORIES:-"default,category"}
  POST_CONTENT=${POST_CONTENT:-"Default content for the post."}
